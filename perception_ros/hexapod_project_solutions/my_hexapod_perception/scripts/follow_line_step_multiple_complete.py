...
class LineFollower(object):
    ...
    def __init__(self, star_color="green", go_right=True, show_crop=False):
        ...
        self.star_HSV_ranges_colour_db = {
                            "green":{"upper":[120,255,255],"lower":[61,120,0]},
                            "red":{"upper":[2,255,255],"lower":[0,211,0]},
                            "blue":{"upper":[116,255,255],"lower":[107,211,0]},
                            "yellow":{"upper":[32,255,255],"lower":[28,212,0]},
                                }
    ...
    def get_current_twist(self):
        return self.current_twist, self.found_color
        ...
        if self.detection_state_on:
            ...
            if upper_hsv_color is not None and lower_hsv_color is not None:
                ...
                try:
                    ...
                    # We divide it in four zones
                    # D,I,X,Y (D and I turn withut linear, X Y is linear without turning X fast y slow)
                    if abs(error_x) < 150:
                        if abs(error_y) > 190:
                            # X Fast Linear, Line far away
                            twist_object.linear.x = 0.4
                            twist_object.angular.z = 0
                        else:
                            # Y Slow Linear, Line far away
                            twist_object.linear.x = 0.2
                            twist_object.angular.z = 0
                    else:
                        # D or I Angular only
                        twist_object.linear.x = 0.0
                        corrected_angular = -error_x / 100
                        sign_v = np.sign([corrected_angular])[0]
                        if sign_v == 1.0:
                            twist_object.angular.z = min(corrected_angular, 0.2)
                        else:
                            twist_object.angular.z = max(corrected_angular, -0.2)
                    ...
                    # rospy.loginfo("[l="+str(self.current_twist.linear.x)+",a="+str(self.current_twist.angular.z)+"]")
                    self.found_color = True

                except Exception as e:
                    if self._star_color == "yellow":
                        print("Exception=="+str(e)+"-"+str(self._star_color))

                    cv2.imshow("RES"+str(self._star_color), res)
...
def main():
    rospy.init_node('line_following_node', anonymous=True, log_level=rospy.DEBUG)
    
    
    line_green_right = LineFollower(star_color="green", go_right=True)
    line_green_right.set_detection_state(True)
